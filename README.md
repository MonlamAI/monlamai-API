...
3. Configure environment variables:
   ```bash
   heroku config:set MODEL_AUTH=your_model_auth_token
...
1. Initialize EB CLI:
   ```bash
   eb init -p python-3.8 monlamai-api
   ```

2. Create an environment:
   ```bash
   eb create monlamai-api-env
   ```

3. Configure environment variables:
   ```bash
   eb setenv MODEL_AUTH=your_model_auth_token API_KEY=your_api_key
   # Set other environment variables as needed
   ```
...
```bash
pytest v1/test/test_translation.py
```
